...
FILE* open_file(char *fname,char *mode) {
  ...
  if (!fh) {
    fprintf(stderr,"INFO  ERROR: failed to open '%s'!\n", fname);
    exit(1);
  }
  return fh;
}
  ...
  */
  ...
  switch(pamnt)
	...
	default:
      ...
      break;
    default:
      fprintf(stderr,"INFO  ERROR: '%c' part of PAM sequence not a valid nucleotide\n", pamnt);
      exit(1);
    }
}
  ...
  */
  switch(seqnt)
    ...
    case 'n':
      ...
      break;      
    default:      
      fprintf(stderr,"INFO  ERROR: '%c' is not valid DNA base symbol!\n", seqnt);
      exit(1);
      return 0;
      break;
    }
...
char baseComplement(char seqnt) {  
  ...
  switch(seqnt)
    ...
    case 'c':
      ...
      break;      
    default:
      fprintf(stderr,"INFO  ERROR: '%c' not valid DNA base symbol!\n", seqnt);
      exit(0);
      break;
    }
}
...
void GRNAsToTrie(trie *T, grna_list *g, int guidelen, int chr_number) {
  ...
  for(i=0;i<g->pos;i++) {
	...
	fprintf(stderr,"INFO  ERROR: Failed to add gRNA sequence detected in chr=%s, pos=%lld, strand=%c. Exiting!",T->chr_names[chr_number],g->starts[i], g->strands[i]);
	exit(1);
      }
    }
    else {
	...
	fprintf(stderr,"INFO  ERROR: Failed to add gRNA sequence in chr=%s, pos=%lld, strand=%c). Exiting!",T->chr_names[chr_number],g->starts[i], g->strands[i]);
	exit(1);
      }
    }
  }
...
void GRNAsToTrieHaplotypes(trie *T, grna_list *g1, grna_list *g2, int guidelen, int chr_number) {
  ...
  while( (g1pos < g1->pos) && (g2pos < g2->pos) ) {
	...
	fprintf(stderr,"INFO  ERROR: Failed to add gRNA sequence detected in chr=%s, pos=%lld, strand=%c. Exiting!",T->chr_names[chr_number],g2->starts[g2pos], g2->strands[g2pos]);
	exit(1);
      }
      g2pos++;
    }
	...
	fprintf(stderr,"INFO  ERROR: Failed to add gRNA sequence detected in chr=%s, pos=%lld, strand=%c. Exiting!",T->chr_names[chr_number],g1->starts[g1pos], g1->strands[g1pos]);
	exit(1);
      }
      g1pos++;
    }
...
grna_list *fastaSequenceToGRNAs(char *header, char *s, char *sr, long long spos, char *pam) {
  ...
  fprintf(stderr,"INFO  %lld gRNA sequences discovered in '%s'.\n",glist->pos, header);
  return glist;
}
...
grna_list *fastaVariantSequenceToGRNAsequences(char *header, char *s, char *sr, long long spos, char *pam) {
  ...
  fprintf(stdout,"INFO  Estimated reference length for '%s' excluding indels: %lld\n",header,realpos);
  return glist;
}
  ...
  */
  ...
  while(!fas->sequence) {
	...
	if ((fas->headerlen+1)==BUFFER_SIZE) {
	  fprintf(stderr,"INFO  ERROR: Fasta header too long! Advice: edit BUFFER_SIZE and re-compile! Exiting!\n");
	  exit(1);
	}
	if (fas->buffer[fas->i] == '\n') {
  ...
  fas->sequence = 0;
  if(fas->header[0] == '\0') {
    fprintf(stderr,"INFO  ERROR: No fasta header. Exiting!\n");
    exit(1);
  }
...
void readFaToTrie(trie *T, char *genomefname, char *pam, char* outputfname, int uppercaseOnly, int printGRNAsOnly) {
  ...
  while(fastaReader(fas)) {
    fprintf(stdout,"INFO  Processing %s (length %lld) in %s ...\n", fas->header, fas->slen, genomefname);
...
void readFaToTrieVCF(trie *T, char *genomefname1, char *genomefname2, char *pam, char* outputfname, int uppercaseOnly, int printGRNAsOnly) {
  ...
  fprintf(stdout,"INFO  Starting gRNA discovery.\n");
  // read first fasta sequence (chromosome) in file
  fprintf(stdout,"INFO  Reading sequence from %s.\n",genomefname1);
  fflush(stdout);
  ...
  while(read_success1 && read_success2) {
    ...
    if (strcmp(fas1->header, fas2->header) != 0) {
      fprintf(stdout,"INFO  ERROR! Chr order in %s and %s not the same: %s != %s. Exiting!\n", genomefname1, genomefname2, fas1->header, fas2->header);
      exit(1);
    }
    
    fprintf(stdout,"INFO  Processing 1st haploid sequence for '%s;.\n", fas1->header);
    fflush(stdout);
    fastaReaderImproveSequence(fas1);
    fprintf(stdout,"INFO  Processing 2nd haploid sequence for '%s'.\n", fas1->header);
    fflush(stdout);
    fastaReaderImproveSequence(fas2);

    fprintf(stdout,"INFO  Running gRNA discovery for 1st haploid sequence of '%s'\n",fas1->header);
    fflush(stdout);
    g1 = fastaVariantSequenceToGRNAsequences(fas1->header, fas1->s, fas1->sr, fas1->slen, pam);
    fprintf(stdout,"INFO  Running gRNA discovery for 2nd haploid sequence of '%s'\n",fas1->header);
    fflush(stdout);
    g2 = fastaVariantSequenceToGRNAsequences(fas2->header, fas2->s, fas2->sr, fas2->slen, pam);

    if(printGRNAsOnly) {
      fprintf(stdout,"INFO  Saving gRNAs in '%s'.\n", fas1->header);
      fflush(stdout);
      printGRNAsHaplotypes(g1, g2, guidelen, outfh);
    }
    else {
      fprintf(stdout,"INFO  Indexing gRNAs in %s.\n", fas1->header);
      fflush(stdout);
