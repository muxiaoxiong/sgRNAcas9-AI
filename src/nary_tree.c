...
void mcontainer_add_nt(mcontainer *m, mcontainer *m_new, int mpos, int seqpos, char nt, int mismatch, int max_mismatches, trieNodeTT* nodeT)
{
  ...
  */
	...
	{
		fprintf(stderr, "INFO ERROR: Not enough space allocated for new approximate sequences. Exiting!\n");
		exit(1);
	}
...
mcontainer *TrieAMatch(trie *T, char array[], int alen, int nmis)
{
	...
	if(alen != T->readlen)
	{
		fprintf(stderr,"INFO ERROR: Depth of the trie and length of the sequence are not the same!\n");
		exit(1);
	}
...
int TrieAMatchSummary(trie *T, char array[], int alen, int nmis)
{
	...
	// print results
	fprintf(stdout,"INFO Container holds these approximate sequences:\n");
	mcontainer_print(m);
