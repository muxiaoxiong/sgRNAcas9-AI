...
void file_accessible(char *fname, int mode) {
  ...
  if ( access (fname, mode) != 0 )
    {
      printf("INFO  ERROR: %s not accessible!\n", fname);
      exit(1);
    }
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "ABCg:o:s:vV:m:t:lhp:")) != -1)
	  {
		...
		if (string_ends_with(optarg,".fa")) { referenceGenomePath = optarg; }
		  ...
		  fprintf(stderr,"INFO  ERROR: file does not appear to be fasta file (.fa).\n");
		  exit(1);
		}
		break;
	      case 'o':
		...
		if (string_ends_with(optarg,".vcf") || string_ends_with(optarg,".VCF")) { vcfPath = optarg; }
		  ...
		  fprintf(stderr,"INFO  ERROR: file does not appear to be vcf file (.vcf).\n");
		  exit(1);
		}
		vcfPath = optarg;
		break;
		...
		if (isprint(optopt))
		  {
		    fprintf(stderr, "INFO  ERROR: Unknown option `-%c'.\n", optopt);
		  }
		else
		  {
		    fprintf(stderr, "INFO  ERROR: Unknown option character `\\x%x'.\n", optopt);
		  }
		return 1;
	      default:
		fprintf(stderr, "INFO  ERROR: Unknown option '-%c'\n", c);
	...
	for (index = optind; index < argc; index++)
	  {
	    fprintf(stderr,"INFO  ERROR: Non-option argument '%s'. Exiting!\n", argv[index]);
	    exit(1);
	  }
	...
	if (!outFile) {
	  fprintf(stderr, "INFO  ERROR: No output defined. Use -o flag. Exiting!\n");
	  usage();
	  exit(1);
	}
