...
int find_chromosome_positions_ref(char* gName, long long* chr_pos, long long* chr_size, char** chr_names)
{
	...
	if (!f)
	  {
	    fprintf(stderr,"INFO Failed to read %s. Exiting!\n", gName);
	    exit(1);
	  }
	...
	fprintf(stdout,"INFO Analysing %s.\n", gName);
	...
	while (read != -1)
	{
		if (line[0] == '>') // new chromosome
		{
			...
			if (pT > CHR_NUMBER_MAX) {
			  fprintf(stderr,"INFO Max number chromosomes reached (%d)! (See CHR_NUMBER_MAX constant in read.h). Exiting!\n\n",CHR_NUMBER_MAX);
			  exit(1);
			}
		}
	...
	fprintf(stderr,"INFO %d sequences for chromosomes and chromosomal fragments identified (%ld seconds).\n", pT, elapsed);
...
int VCF_to_genome(char* gName, char* vcfName, char* newName, char* newName2)
{
	...
	fprintf(stdout,"INFO Creating phased genomes.\n");
	...
	for (int i = 0; i < n; i++)
	{
		...
		chr[x-1] = '\0';
		fprintf(stdout,"INFO Adding variants to '%s'\n", chr);
		...
		if (j==m)
		{
		        fprintf(stdout,"INFO WARNING: No variants for %s!\n", chr);
			int write = fwrite(buffer,chr_size[i],1,f_new);
			if (write == -1) { fprintf(stderr,"INFO ERROR: Writing to %s failed. Exiting!\n", newName2); exit(1); }
...
((vcf->hap[0] == 1 && old_hap[0] == 1) || (vcf->hap[2] == 1 && old_hap[2] == 1) || phased == 0))
	...
	if (phased == 1) {fprintf(stdout,"INFO Phased genomes created! Sequences saved in %s (%d variants) and %s (%d variants). Phasing run time: %f seconds.\n", newName, nb_of_var_1, newName2, nb_of_var_2, total_time); }
	else {fprintf(stdout,"INFO A modified genome incorporating variant data (%d variants) saved in %s. Phasing run time: %f seconds.\n", nb_of_var_1, newName, total_time);}
